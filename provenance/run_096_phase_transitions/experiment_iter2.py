...
def compute_gradient_coherence_index(model, data, target, loss_fn, epsilon=1e-5):
    ...
    # Project gradients to common dimension for comparison
    ...
    for g in grads:
        ...
        if len(flat_g) > proj_dim:
            ...
            # Use deterministic projection based on layer shape for consistency
            torch.manual_seed(hash(tuple(g.shape)) % 10000)
            proj_matrix = torch.randn(len(flat_g), proj_dim, device=g.device) / np.sqrt(proj_dim)
            proj_g = flat_g @ proj_matrix
        else:
            # Pad with zeros to reach proj_dim
            proj_g = F.pad(flat_g, (0, proj_dim - len(flat_g)))
    ...
    for i in range(n_layers):
        for j in range(n_layers):
            if i == j:
                ...
                # Now both gradients have the same dimension
...
def run_experiment():
    ...
    if len(collapse_mags) >= 2 and abs(corr) > 0.3:
        signal_detected = True
        signal_description = f"Collapse magnitude correlates with generalization (r={corr:.3f}, p={p_value:.3f})"
    elif len(collapse_mags) > 0:
        signal_description = "Collapse detected but insufficient samples for correlation"
    else:
        signal_description = "No coherence collapse detected"
    ...
    if method_mean > random_mean * 5:  # Significantly better than random
        if signal_detected:
            ...
            print(f"NO_SIGNAL: {signal_description}")
    else:
