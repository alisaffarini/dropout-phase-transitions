...
% Suppress the page-1 notice footer entirely (no venue/header text).
\makeatletter
...
\icmltitlerunning{Dropout as a Phase Transition}

\begin{document}

\twocolumn[
\icmltitle{Dropout as a Phase Transition: Critical Points in\\Neural Network Regularization Reveal Model--Task Fit Regimes}
\begin{icmlauthorlist}
\icmlauthor{Ali Saffarini}{harvard}
\end{icmlauthorlist}
\icmlaffiliation{harvard}{Harvard University}
\icmlcorrespondingauthor{Ali Saffarini}{alisaffarini@college.harvard.edu}
\vskip 0.3in
]
...
\begin{align*}
    \chi_{\max}^{\text{MLP}}    &= 1.4 \pm 0.0 \\
    \chi_{\max}^{\text{CNN}}    &= 4.4 \pm 0.2 \\
    \chi_{\max}^{\text{VGG}}    &= 7.0 \pm 0.8 \\
    \chi_{\max}^{\text{ResNet}} &= 7.2 \pm 0.3
\end{align*}
